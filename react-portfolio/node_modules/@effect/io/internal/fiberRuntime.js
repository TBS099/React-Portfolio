...
var _effect = /*#__PURE__*/require("@effect/io/internal/effect");
...
class FiberRuntime {
  ...
  [OpCodes.OP_FAILURE](op) {
    ...
    const cause = List.isNil(span) ? op.i0 : internalCause.annotated(op.i0, internalCause.makeSpanAnnotation(List.unsafeHead(span)));
...
const tracerLogger = /*#__PURE__*/internalLogger.makeLogger(({
  ...
  span.value.event(message, clockService.value.unsafeCurrentTimeNanos(), attributes);
});
/** @internal */
...
const acquireRelease = /*#__PURE__*/(0, _Function.dual)(args => core.isEffect(args[0]), (acquire, release, options) => {
  if (options?.interruptible) {
    return ensuring(acquire, addFinalizer(exit =>
    // @ts-expect-error
    release(exit)));
  }
  return core.uninterruptible(core.tap(acquire, a => addFinalizer(exit =>
  // @ts-expect-error
  release(a, exit))));
});
/* @internal */
exports.acquireRelease = acquireRelease;
const addFinalizer = finalizer => core.flatMap(core.context(), context => core.flatMap(scope, scope => core.scopeAddFinalizerExit(scope, exit => core.asUnit(core.provideContext(context)(finalizer(exit))))));
/* @internal */
...
const raceAwait = /*#__PURE__*/(0, _Function.dual)(2, (self, that) => core.fiberIdWith(parentFiberId => raceWith(self, {
  ...
  onSelfDone: (exit, right) => core.exitMatchEffect(exit, {
    onFailure: cause => (0, _effect.mapErrorCause)(cause2 => internalCause.parallel(cause, cause2))(internalFiber.join(right)),
    onSuccess: value => core.as(value)(core.interruptAsFiber(parentFiberId)(right))
  }),
  onOtherDone: (exit, left) => core.exitMatchEffect(exit, {
    onFailure: cause => (0, _effect.mapErrorCause)(cause2 => internalCause.parallel(cause2, cause))(internalFiber.join(left)),
...
const invokeWithInterrupt = (dataSource, all) => core.fiberIdWith(id => core.flatMap(core.flatMap(forkDaemon(core.interruptible(dataSource)), processing => core.asyncInterrupt(cb => {
